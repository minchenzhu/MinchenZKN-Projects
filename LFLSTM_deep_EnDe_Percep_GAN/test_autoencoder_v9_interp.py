...
# configuration
import config_autoencoder_y_3d as hp
...
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_sideboard", "s4" ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_herbs", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_cotton", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_bicycle", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_tomb", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_platonic", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_rosemary", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_origami", "s4"  ),
# ( "super_resolution", "benchmark", "not seen", "lf_test_benchmark_bedroom", "s4"  ),

# ( "super_resolution", "stanford", "not seen", "lf_test_stanford_HR_Bunny", "s4" ),
( "super_resolution", "stanford", "not seen", "lf_test_stanford_Eucalyptus", "s2" ),
...
# ( "super_resolution", "stanford", "not seen", "lf_test_stanford_Truck", "s4" ),

# ( "super_resolution", "HCI", "not seen", "lf_test_HCI_maria", "s4" ),
...
# ( "super_resolution", "lytro", "not seen", "lf_test_lytro_hedgehog3", "s4" ),
# ( "super_resolution", "lytro", "not seen", "lf_test_lytro_koala", "s4" ),
# ( "super_resolution", "lytro", "not seen", "lf_test_lytro_origami", "s4" ),
# ( "super_resolution", "lytro", "not seen", "lf_test_lytro_owl_str", "s4" ),
# ( "super_resolution", "lytro", "not seen", "lf_test_lytro_owl2", "s4" ),

# ( "super_resolution", "stanford", "seen", "lf_test_stanford_HR_Bunny", "s4" ),
...
for lf_name in data_folders:
    ...
    LF_net_in = LF_gt['input']
    sh_input = LF_net_in.shape
    # process shapes
    if sh_input[2] % 48 !=0:
        y_pad = 48 - sh_input[2] % 48
    else:
        y_pad = 0

    if sh_input[3] % 48 !=0:
        x_pad = 48 - sh_input[3] % 48
    else:
        x_pad = 0

    LF_net_in = np.pad(LF_net_in, ((0,0),(0,0),(0,y_pad),(0,x_pad),(0,0)), mode = 'constant')
    ...
    if color_space == 'YCBCR':
        ...
        result_lf = encode_decode_lightfield(data, LF_net_in,
                                             ...
                                             decoder_path=decoder_path, scales=scales, target_channel = 0)
        lf_out2 = dict()
        ...
        if grey:
            # result_lf_cb = encode_decode_lightfield(data, LF_net_in,
            #                                      inputs, outputs, color_space, lf_name[-1],
            #                                      decoder_path=decoder_path, scales=scales, target_channel = 1)
            # lf_out2_cb = dict()
            # for i in range(0, len(scales)):
            #     lf_out2_cb[scales[i]] = result_lf_cb[2][scales[i]]
            #     mask = result_lf_cb[1][scales[i]]
            #     lf_out2_cb[scales[i]] = scale_back(lf_out2_cb[scales[i]], mask)
            #
            # result_lf_cr = encode_decode_lightfield(data, LF_net_in,
            #                                      inputs, outputs, color_space, lf_name[-1],
            #                                      decoder_path=decoder_path, scales=scales, target_channel = 2)
            # lf_out2_cr = dict()
            # for i in range(0, len(scales)):
            #     lf_out2_cr[scales[i]] = result_lf_cr[2][scales[i]]
    ...
    if 's2' in scales:
        file = h5py.File(result_folder + lf_name[3] +  '_s2.hdf5', 'w')

        dset_LF_input = file.create_dataset('lf_in', data=LF_gt['input'])
        dset_LF_out2 = file.create_dataset('lf_out2', data=lf_out4['s2'])
        dset_LF_out_gt2 = file.create_dataset('lf_in2', data=LF_crosshair['input_s2'])
        # scipy.misc.imsave(lf_name[-2] + '_2_.png', lf_out4['s2'][0,4,:,:,:])
        # scipy.misc.imsave(lf_name[-2] + '_2_gt.png', LF_crosshair['input_s2'][0, 4, :, :, :])

    if 's4' in scales:
        # file = h5py.File(result_folder + lf_name[3] + '_s4.hdf5', 'w')
        file = h5py.File(result_folder + lf_name[3] + '_sBig.hdf5', 'w')
        dset_LF_input = file.create_dataset('lf_in', data=LF_gt['input'])
        dset_LF_out4 = file.create_dataset('lf_out4', data=lf_out4['s4'])
        # scipy.misc.imsave(lf_name[-2] + '_4_.png', lf_out4['s4'][0, 4, :, :, :])
        # scipy.misc.imsave(lf_name[-2] + '_4_gt.png', LF_crosshair['input_s4'][0, 4, :, :, :])
        # dset_LF_out_gt4 = file.create_dataset('lf_in4', data=LF_crosshair['input_s4'])

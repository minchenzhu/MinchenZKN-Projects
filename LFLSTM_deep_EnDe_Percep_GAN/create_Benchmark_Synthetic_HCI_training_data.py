...
training_data_dir = "/home/mz/HD_data/LFLSTM_data/"
training_data_filename = 'lflstm_patch_try_48_v2.hdf5'
...
dset_LF = file.create_dataset('lf_patches', (nviews, nviews, py_LR_s4, px_LR_s4, 3, 1),
                                    chunks = (nviews, nviews, py_LR_s4, px_LR_s4, 3, 1),
                                    maxshape = (nviews, nviews, py_LR_s4, px_LR_s4, 3, None))

# dset_HS = file.create_dataset('lf_hid_state', (py_LR_s4, px_LR_s4, h_dim,1),
#                                     chunks = (py_LR_s4, px_LR_s4, h_dim,1),
#                                     maxshape = (py_LR_s4, px_LR_s4, h_dim,None))
#
# dset_LF = file.create_dataset('lf_patches', (nviews,nviews, py, px, 3, 1),
#                                     chunks = (nviews,nviews, py, px, 3, 1),
#                                     maxshape = (nviews,nviews, py, px, 3, None))
        ...
        for lf_name in data_folders:
            ...
            for by in np.arange(0, cy_HR):
                ...
                for bx in np.arange(0, cx_HR):
                    ...
                    lf_patch = lf_tools.lf_patches(LF_temp, y_LR_s4, x_LR_s4, py_LR_s4, px_LR_s4)
                    # make sure the direction of the view shift is the first spatial dimension
                    # stack_h_HR = np.transpose(stack_h_HR, (0, 2, 1, 3))
                    # patches = lf_patch.reshape([nviews * nviews, py_LR_s4, px_LR_s4, channel])
                    # write to respective HDF5 datasets
                    dset_LF.resize(index + 1, 5)
                    dset_LF[:, :, :, :, :, index] = lf_patch
                    # dset_HS.resize(index + 1, 3)
                    # dset_HS[:, :, :, index] = np.ones([py_LR_s4, px_LR_s4, h_dim])/10
                    # write to respective HDF5 datasets
                    # dset_LF.resize(index + 1, 5)
                    # dset_LF[:, :, :, :, :, index] = lf_patch

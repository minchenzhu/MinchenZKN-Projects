...
def encode_decode_lightfield(data, LF, inputs, outputs, ColorSpace, lf_scale, decoder_path, scales, target_channel = 0):
  # light field size
  H = LF.shape[2]
  W= LF.shape[3]
  ...
  LF_crosshair = dict()
  if 's2' in scales:
    mask_sum['s2'] = np.zeros([np.int(2*H), np.int(2*W)], dtype=np.float32)
    LF_crosshair['s2'] = np.zeros([2, 9, np.int(2*H), np.int(2*W), num_channels], dtype=np.float32)
  if 's4' in scales:
    mask_sum['s4'] = np.zeros([np.int(4*H), np.int(4*W)], dtype=np.float32)
    LF_crosshair['s4'] = np.zeros([2, 9, np.int(4*H), np.int(4*W), num_channels], dtype=np.float32)
  ...
  for py in range(by):
    ...
    for px in range(bx):
      ...
      if ColorSpace == 'YCBCR':
        patch['stack_v'] = rgb2YCbCr(np.clip(patch[ 'stack_v' ] , 0.0, 1.0))
        patch['stack_h'] = rgb2YCbCr(np.clip(patch[ 'stack_h' ] , 0.0, 1.0))
        # patch['stack_v'] = np.clip(patch[ 'stack_v' ] , 0.0, 1.0)
        # patch['stack_h'] = np.clip(patch[ 'stack_h' ] , 0.0, 1.0)
        patch['stack_v'][...,0:1] = patch['stack_v'][...,target_channel:target_channel+1]
        patch['stack_h'][..., 0:1] = patch['stack_h'][..., target_channel:target_channel + 1]
